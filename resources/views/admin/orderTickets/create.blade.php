    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.order-tickets.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah_tiket') ? 'is-invalid' : '' }}" type="number" name="jumlah_tiket" id="jumlah_tiket" value="{{ old('jumlah_tiket', '') }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('tanggal_pemesanan') ? 'is-invalid' : '' }}" type="text" name="tanggal_pemesanan" id="tanggal_pemesanan" value="{{ old('tanggal_pemesanan') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status_tiket') ? 'is-invalid' : '' }}" name="status_tiket" id="status_tiket">
                    ...
                    @foreach(App\Models\OrderTicket::STATUS_TIKET_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status_tiket', 'aktif') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
